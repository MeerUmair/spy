...
class EntryExecTranJcbPreca {
	 ...
	 * JCBプリカ決済登録・決済を実行する
	 *
	 * @param EntryExecTranJcbPrecaInput $input    JCBプリカ決済登録・決済入力パラメータ
	 * @return  EntryExecTranJcbPrecaOutput JCBプリカ決済登録・決済出力パラメータ
	 * @exception GPayException
	 */
	function exec(&$input) {
		// JCBプリカ決済取引登録入力パラメータを取得
		$entryTranJcbPrecaInput =& $input->getEntryTranJcbPrecaInput();
		// JCBプリカ決済決済実行入力パラメータを取得
		$execTranJcbPrecaInput =& $input->getExecTranJcbPrecaInput();

		// JCBプリカ決済登録・決済出力パラメータを生成
		...
		if(!$entryFlg){
			// JCBプリカ決済登録出力パラメータを生成
			$entryTranJcbPrecaOutput = new EntryTranJcbPrecaOutput();

			$entryTranJcbPrecaOutput->setAccessId($accessId);
			$entryTranJcbPrecaOutput->setAccessPass($accessPass);
